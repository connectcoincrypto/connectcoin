...
class WalletTaprootTest(BitcoinTestFramework):
    ...
    def do_test_sendtoaddress(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        for i in range(4):
            ...
            if treefn is not None:
                ...
                assert_equal(addr_g, addr_r)
            boring_balance = int(self.boring.getbalance() * COIN)
            to_amnt = random.randrange(1000000, boring_balance)
            self.boring.sendtoaddress(address=addr_g, amount=Decimal(to_amnt) / COIN, subtractfeefromamount=True)
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
            test_balance = int(rpc_online.getbalance() * COIN)
            ret_amnt = random.randrange(100000, test_balance)
            # Increase fee_rate to compensate for the wallet's inability to estimate fees for script path spends.
            res = rpc_online.sendtoaddress(address=self.boring.getnewaddress(), amount=Decimal(ret_amnt) / COIN, subtractfeefromamount=True, fee_rate=200)
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
    ...
    def do_test_psbt(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        for i in range(4):
            ...
            self.boring.sendtoaddress(address=addr_g, amount=Decimal(to_amnt) / COIN, subtractfeefromamount=True)
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
            test_balance = int(psbt_online.getbalance() * COIN)
            ret_amnt = random.randrange(100000, test_balance)
            # Increase fee_rate to compensate for the wallet's inability to estimate fees for script path spends.
            psbt = psbt_online.walletcreatefundedpsbt([], [{self.boring.getnewaddress(): Decimal(ret_amnt) / COIN}], None, {"subtractFeeFromOutputs":[0], "fee_rate": 200, "change_type": address_type})['psbt']
