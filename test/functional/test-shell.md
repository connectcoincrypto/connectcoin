...
* Manage regtest connectcoind subprocesses.
* Access RPC interfaces of the underlying connectcoind instances.
...
* Python3
* `connectcoind` built in the same repository as the `TestShell`.
...
```
>>> import sys
>>> sys.path.insert(0, "/path/to/connectcoin/build/test/functional")
>>> from test_framework.test_shell import TestShell
```

The following `TestShell` methods manage the lifetime of the underlying connectcoind
...
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Initializing test directory /path/to/connectcoin_func_test_XXXXXXX
```
...
temporary folder. If you need more connectcoind nodes than set by default (1),
...
Connectcoind subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `connectcoind` node
...
useful in constructing objects which can be passed to the connectcoind nodes managed
by a running `TestShell` object.

## 5. Shutting the `TestShell` down

Shutting down the `TestShell` will safely tear down all running connectcoind
instances and remove all temporary data and logging directories.

```
>>> test.shutdown()
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Stopping nodes
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Cleaning up /path/to/connectcoin_func_test_XXXXXXX on exit
...
>>> test.shutdown()
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Stopping nodes
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Not cleaning up dir /path/to/connectcoin_func_test_XXXXXXX on exit
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Tests successful
```

The following utility consolidates logs from the connectcoind nodes and the
underlying `BitcoinTestFramework`:

* `/path/to/connectcoin/build/test/functional/combine_logs.py
  '/path/to/connectcoin_func_test_XXXXXXX'`
