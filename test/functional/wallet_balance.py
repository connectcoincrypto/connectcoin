...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[0].getbalance("*"), 15)
        assert_equal(self.nodes[0].getbalance("*", 1), 15)
        assert_equal(self.nodes[0].getbalance(minconf=1), 15)
        assert_equal(self.nodes[0].getbalance(minconf=0), 15)
        ...
        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 18, [Decimal('0.01'), Decimal('0.02')])
        ...
        # Before `test_balance()`, we have had two nodes with a balance of 15
        # each and then we:
        #
        # 1) Sent 12 from node A to node B with fee 0.01
        # 2) Sent 18 from node B to node A with fee 0.01
        #
        ...
        # a balance of 11 on node B as unconfirmed and trusted.
        #
        ...
        def test_balances(*, fee_node_1=0):
            ...
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('2.99'),  # change from node 0's send
                                                 'untrusted_pending': Decimal('18.0'),
                                                 'nonmempool':        Decimal('0.0')}}
            expected_balances_1 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0E-8'),  # node 1's send had an unsafe input
                                                 'untrusted_pending': Decimal('9.0') - fee_node_1,  # Doesn't include output of node 0's send since it was spent
                                                 'nonmempool':        Decimal('0.0')}}
            ...
            assert_equal(self.nodes[0].getbalance(), Decimal('2.99'))  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('2.99'))
        ...
        balance_node0 = Decimal('20.99')  # node 1's send plus change from node 0's send
        balance_node1 = Decimal('8.98')  # change after node 1's send
        ...
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), Decimal('8.97'), [Decimal('0.01')])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
        ...
        for _ in range(3):
            ...
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), 29)
        ...
        tx_replace = tx_orig.replace(
            (29 * COIN).to_bytes(8, "little", signed=True).hex(),
            (28 * COIN).to_bytes(8, "little", signed=True).hex(),
        )
