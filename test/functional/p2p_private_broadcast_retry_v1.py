...
* Get one successful connection to a Tor peer (.onion) so that connectcoind assumes the configured
...
class P2PPrivateBroadcastRetryV1(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # The destinations behind the -proxy= don't actually support v2. When connectcoind runs with -v2transport=1
        ...
        self.log.info("Opening a connection to a Tor addresses, so connectcoind considers -onion= is a real Tor proxy")
