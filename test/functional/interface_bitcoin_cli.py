...
# The block reward of coinbaseoutput.nValue (15) CC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 15 CC/block.
BLOCKS = COINBASE_MATURITY + 1
BALANCE = (BLOCKS - COINBASE_MATURITY) * 15
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"connectcoin-cli -h\" for help or \"connectcoin-cli listwallets\" to see "
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `connectcoin-cli getblockchaininfo`")
        ...
        # Prevent connectcoin-cli from using existing rpcport in conf
        ...
        assert_raises_process_error(1, "Cookie file credentials were invalid and no rpcpassword was specified.", self.nodes[0].cli(f"-rpccookiefile={self.nodes[0].datadir_path / 'connectcoin.conf'}").echo)
        ...
        self.log.info("Test -getinfo and connectcoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            ...
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (CC/kvB)']), network_info['relayfee'])
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)

            # Setup to test -getinfo, -generate, and -rpcwallet= with multiple wallets.
            wallets = [self.default_wallet_name, 'Encrypted', 'secret']
            amounts = [None, Decimal(1), Decimal(4)]
            ...
            # Mine a block to confirm; adds a block reward (15 CC) to the default wallet.
            ...
            # Test connectcoin-cli -generate with -rpcwallet in multiwallet mode.
        ...
        if not self.is_ipc_compiled():
            ...
            self.log.info("Test connectcoin-cli -ipcconnect triggers error if not built with IPC support")
            # node.cli.options includes -rpcconnect which can't be combined with -ipcconnect, so pass just -datadir directly to keep connectcoin-cli on the test's connectcoin.conf
            args = self.nodes[0].binaries.valgrind_cmd + [self.nodes[0].binaries.paths.connectcoin_cli, f"-datadir={self.nodes[0].datadir_path}", "-ipcconnect=unix", "-getinfo"]
            result = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            assert_equal(result.stdout, "error: connectcoin-cli was not built with IPC support\n")
