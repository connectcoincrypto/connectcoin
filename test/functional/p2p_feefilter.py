...
class FeeFilterTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # See issue #16499
        self.extra_args = [[
            "-minrelaytxfee=0.0000000100",
            "-mintxfee=0.0000000100"
        ]] * self.num_nodes
    ...
    def test_feefilter(self):
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.0000000200'), from_node=node1)['wtxid'] for _ in range(3)]
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.0000000150'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()

        self.log.info("Test txs paying 0.1 sat/byte are no longer received by test connection")
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.0000000100'), from_node=node1)['wtxid'] for _ in range(3)]
        ...
        # as well.
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.0000020000'), from_node=node0)['wtxid'] for _ in range(1)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
        ...
        txids = [miniwallet.send_self_transfer(fee_rate=Decimal('0.0000020000'), from_node=node1)['wtxid'] for _ in range(3)]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
