...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def test_init(self, node, wallet_names):
        ...
        assert_equal(set(node.listwallets()), {"w4", "w5"})
        w5 = get_wallet(node, "w5")
        assert_equal(w5.getbalances()["mine"]["immature"], 15)
    ...
    def test_balances_and_fees(self, node, wallet_names, in_wallet_dir):
        ...
        for wallet_name, wallet in zip(wallet_names, wallets):
            ...
            assert_equal(wallet.getbalances()["mine"]["immature"], 15 if wallet is wallets[0] else 0)
