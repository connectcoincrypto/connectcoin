...
to a hash that has been compiled into connectcoind.
...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def test_invalid_snapshot_scenarios(self, valid_snapshot_path):
        ...
        cases = [
            ...
            [b"\xff" * 32, 0, "e6391bfa616f09d9b45d4632f300d9042cbb9d8485f5addf9601cbf1c65a625e", None],  # wrong outpoint hash
            ...
            [b"\x01", 33, "e168a9e29eb9f01b78d1bdcd92f802620c8c531de771b344bf6f8558358fd7ea", None],  # wrong outpoint index
            [b"\xff", 34, None, "Bad snapshot data after deserializing 0 coins."],  # wrong coin code VARINT
            [b"\x2a", 34, "14219ba897e650d7d1ee9fb87f69fed0060f4bd9374c2fe60c225750e0e17025", None],  # another parsable coin code
        ...
        for content, offset, wrong_hash, custom_message in cases:
            # The oversized-amount encoding is longer than the valid coin's
            # original amount and script fields (3 bytes). Replace the
            # original fields without consuming bytes from the next coin.
            replace_len = 3 if custom_message is not None and "bad tx out value" in custom_message else len(content)
            with open(bad_snapshot_path, "wb") as f:
                ...
                f.write(content)
                f.write(valid_snapshot_contents[(5 + 2 + 4 + 32 + 8 + offset + replace_len):])

            msg = custom_message if custom_message is not None else f"Bad snapshot content hash: expected f08765d5aac8a793b35d644120aadc280317e0c1118f3f3ee7eaf8c958e854aa, got {wrong_hash}."
            expected_error(msg)

    def test_headers_not_synced(self, valid_snapshot_path):
        for node in self.nodes[1:]:
            msg = "Unable to load UTXO snapshot: The base block header (3543b4c9d2b4f69bd34634920756b94c2c1783ea43a7aea030270f800349f25f) must appear in the headers chain. Make sure all headers are syncing, and call loadtxoutset again."
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test connectcoind should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test connectcoind should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test connectcoind should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
    ...
    def test_snapshot_block_invalidated(self, dump_output_path):
        ...
        for height in [SNAPSHOT_BASE_HEIGHT, SNAPSHOT_BASE_HEIGHT - 1]:
            ...
            msg = "Unable to load UTXO snapshot: The base block header (3543b4c9d2b4f69bd34634920756b94c2c1783ea43a7aea030270f800349f25f) is part of an invalid chain."
            assert_raises_rpc_error(-32603, msg, node.loadtxoutset, dump_output_path)
    ...
    def run_test(self):
        ...
        assert_equal(
            ...
            "3bc4555f3d55422755852fd508144611e4a16942e694a7d3ef743e282395c288")
