...
#ifndef CONNECTCOIN_WALLET_WALLET_H
#define CONNECTCOIN_WALLET_WALLET_H
...
//! Discourage users from setting fees higher than this amount (in connects) per kB.
inline constexpr CAmount HIGH_TX_FEE_PER_KB{COIN / 100};
//! -maxtxfee will warn if called with a higher fee than this amount (in connects).
...
struct CRecipient
{
    ...
    std::optional<PayToDomainOutput> p2c;
};
...
public:
     ...
     * connectcoin-wallet tool code or by wallet migration code. If this is called
    ...
    /** Absolute maximum transaction fee (in connects) used by default for the wallet. */
...
#endif // CONNECTCOIN_WALLET_WALLET_H
