...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.GetBoolArg("-version", false)) {
        std::string strUsage = strprintf("%s connectcoin-wallet utility version", CLIENT_NAME) + " " + FormatFullVersion() + "\n";
...
MAIN_FUNCTION
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `connectcoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
