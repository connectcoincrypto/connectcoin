This directory contains the source code for the ConnectCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the ConnectCoin UI.
...
* `bitcoinunits.(h/cpp)`: ConnectCoin display-unit handling
* `callback.h`
...
4. Enter `connectcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `connectcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `connectcoin-qt`
9. You're all set! Start developing, building, and debugging the ConnectCoin Core GUI
