...
// A fee amount that is above 1 con/vB but below 5 con/vB for most transactions created within these
// unit tests.
...
inline CTransactionRef create_placeholder_tx(size_t num_inputs, size_t num_outputs)
{
    ...
    for (size_t o{0}; o < num_outputs; ++o) {
        ...
        mtx.vout[o].SetP2PK(XOnlyPubKey{GenerateRandomKey().GetPubKey()});
    }
...
{
    const CTransactionRef ptx_1{create_placeholder_tx(1, 1)};
    const CTransactionRef ptx_2{create_placeholder_tx(1, 2)};
    CMutableTransaction witness_tx{*create_placeholder_tx(2, 1)};
    witness_tx.vin.front().scriptWitness.stack.push_back({1});
    const CTransactionRef ptx_3{MakeTransactionRef(witness_tx)};

    std::vector<Wtxid> sorted_wtxids{
        ptx_1->GetWitnessHash(), ptx_2->GetWitnessHash(), ptx_3->GetWitnessHash()};
    std::sort(sorted_wtxids.begin(), sorted_wtxids.end(), [](const Wtxid& lhs, const Wtxid& rhs) {
        return lhs.GetHex() < rhs.GetHex();
    });
    HashWriter writer;
    for (const auto& wtxid : sorted_wtxids) writer << wtxid;
    const uint256 calculated_hash_123{writer.GetSHA256()};

    // All permutations of the package have the same hash, ordered by wtxid.
...
BOOST_AUTO_TEST_CASE(package_validation_tests)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    ...
                                                    /*output_amount=*/CAmount(14 * COIN), /*submit=*/false);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/CAmount(13 * COIN), /*submit=*/false);
    ...
    // 24 Parents and 1 Child
    {
        ...
        for (int i{0}; i < 24; ++i) {
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, CAmount(13 * COIN), false));
...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
    ...
    // missing inputs, so the package validation isn't expected to happen.
    {
        ...
        if (auto err_parent_invalid{CheckPackageMempoolAcceptResult(package_invalid_parent, result_quit_early, /*expect_valid=*/false, m_node.mempool.get())}) {
            ...
            BOOST_CHECK_EQUAL(it_parent->second.m_state.GetResult(), TxValidationResult::TX_INPUTS_NOT_STANDARD);
            BOOST_CHECK_EQUAL(it_parent->second.m_state.GetRejectReason(), "bad-txns-nonstandard-inputs");
    ...
    // In-mempool parent and child with missing parent.
    {
        ...
        auto tx_child_missing_parent = MakeTransactionRef(CreateValidMempoolTransaction({tx_parent_1, tx_parent_2},
                                                                                        ...
                                                                                        {{29 * COIN, child_locking_script}}, /*submit=*/false));
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/parent_locking_script,
                                                    /*output_amount=*/CAmount(15 * COIN) - high_fee, /*submit=*/false);
    CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   /*input_height=*/101, /*input_signing_key=*/parent_key,
                                                   /*output_destination=*/child_locking_script,
                                                   /*output_amount=*/CAmount(15 * COIN) - 2 * high_fee, /*submit=*/false);
    CTransactionRef tx_child = MakeTransactionRef(mtx_child);
    ...
    auto mtx_single_low_fee = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/single_locking_script,
                                                    /*output_amount=*/CAmount(14 * COIN - 1), /*submit=*/false);
...
// the mempool.
#if 0 // Bitcoin witness-script replacement has no typed-output equivalent.
BOOST_AUTO_TEST_CASE(package_witness_swap_tests)
{
    ...
    mtx_child1.vout[0].nValue = CAmount(13 * COIN);
    ...
    auto mtx_parent1 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                     ...
                                                     /*output_amount=*/CAmount(14 * COIN), /*submit=*/true);
    ...
    mtx_parent2_v1.vout[0].nValue = CAmount(13 * COIN);
    ...
    BOOST_CHECK(m_node.mempool->GetMinRelayFee().GetFee(GetVirtualTransactionSize(*ptx_parent3)) <= low_fee_amt);
    ...
    mtx_mixed_child.vout.emplace_back((13 + 14 + 15 - 1) * COIN, mixed_child_spk);
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    const CAmount coinbase_value{15 * COIN};
    ...
    // The mempool minimum feerate is 5 con/vB, but this package just pays 800 connects total.
    ...
    BOOST_CHECK(m_node.mempool->GetMinRelayFee().GetFee(GetVirtualTransactionSize(*tx_parent_cheap)) <= parent_fee);
...
BOOST_AUTO_TEST_CASE(package_rbf_tests)
{
    ...
    // Test package rbf.
    {
        ...
        CTransactionRef tx_child_3 = MakeTransactionRef(CreateValidMempoolTransaction(
            ...
            child_key, child_spk, coinbase_value - 199 - 5000, /*submit=*/false));
        ...
        // 1 parent paying 199 atoms, 1 child paying 5000 atoms. Typed P2PK
        // transactions are larger, so the child needs more sponsorship fee.
        ...
        BOOST_CHECK_EQUAL(it_parent_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 199 + 5000);
        BOOST_CHECK_EQUAL(it_child_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 199 + 5000);
        ...
        m_node.mempool->PrioritiseTransaction(tx_child_1->GetHash(), 6000);
