...
//
// The connectcoin-chainstate executable serves to surface the dependencies required
// by a program wishing to use ConnectCoin Core's consensus engine as it is right
// now.
//
// DEVELOPER NOTE: Since this is a "demo-only", experimental, etc. executable,
//                 it may diverge from ConnectCoin Core's coding style.
//
...
{
    stream
        << "Usage: " << executable << " [-regtest] DATADIR" << std::endl
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 2) {
        ...
        if (option == "-?" || option == "-h" || option == "-help" || option == "--help") {
