...
//! Specify whether a parameter should be parsed by connectcoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make connectcoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `connectcoin-cli` attempt to parse the
 ...
 * `connectcoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make connectcoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `connectcoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so connectcoin-cli
...
static const CRPCConvertParam vRPCConvertParams[] =
{
    ...
    { "sendtop2c", 0, "domain", ParamFormat::STRING },
    { "sendtop2c", 1, "amount" },
    { "sendtop2c", 2, "work" },
    { "sendtop2c", 3, "output_count" },
    { "sendtop2c", 4, "root_certificates_version" },
    { "sendtop2c", 5, "comment", ParamFormat::STRING },
    { "sendtop2c", 6, "comment_to", ParamFormat::STRING },
    { "sendtop2c", 7, "subtractfeefromamount" },
    { "sendtop2c", 8, "replaceable" },
    { "sendtop2c", 9, "conf_target" },
    { "sendtop2c", 10, "estimate_mode", ParamFormat::STRING },
    ...
    { "preparep2cclaim", 1, "vout" },
    { "preparep2cclaim", 2, "fee_rate" },
    { "preparep2cclaim", 3, "proof_size" },
 ...
 *
 * For example, the command `connectcoin-cli -named createwallet "my=wallet"`,
