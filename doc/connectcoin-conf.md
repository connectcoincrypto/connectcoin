# `connectcoin.conf` Configuration File
...
The configuration file is not automatically created; you can create it using your favorite text editor. By default, the configuration file name is `connectcoin.conf` and it is located in the ConnectCoin data directory, but both the ConnectCoin data directory and the configuration file path may be changed using the `-datadir` and `-conf` command-line options.

The `includeconf=<file>` option in the `connectcoin.conf` file can be used to include additional configuration files.

### Default configuration file locations

Operating System | Data Directory | Example Path
-- | -- | --
Windows | `%LOCALAPPDATA%\ConnectCoin\` | `C:\Users\username\AppData\Local\ConnectCoin\connectcoin.conf`
Linux | `$HOME/.connectcoin/` | `/home/username/.connectcoin/connectcoin.conf`
macOS | `$HOME/Library/Application Support/ConnectCoin/` | `/Users/username/Library/Application Support/ConnectCoin/connectcoin.conf`

An example configuration file can be generated by [contrib/devtools/gen-connectcoin-conf.sh](../contrib/devtools/gen-connectcoin-conf.sh).
Run this script after compiling to generate an up-to-date configuration file.
The output is placed under `share/examples/connectcoin.conf`.
To use the generated configuration file, copy the example file into your data directory and edit it there, like so:

```
# example copy command for linux user
cp share/examples/connectcoin.conf ~/.connectcoin
```
