# CJDNS support in ConnectCoin Core

It is possible to run ConnectCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of ConnectCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the ConnectCoin network and individual nodes.
...
## Run ConnectCoin Core with CJDNS

Once you are connected to the CJDNS network, the following ConnectCoin Core
...
When enabled, this option tells ConnectCoin Core that it is running in an
...
IPv6 local network. This helps ConnectCoin Core perform better address management:
...
You can use `connectcoin-cli -addrinfo` to see the
...
## CJDNS-related information in ConnectCoin Core

There are several ways to see your CJDNS address in ConnectCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `connectcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `connectcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `connectcoin-cli help getnodeaddresses` for details.
