...
The data directory is the default location where the ConnectCoin files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.connectcoin/`
macOS    | `$HOME/Library/Application Support/ConnectCoin/`
Windows  | `%LOCALAPPDATA%\ConnectCoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `connectcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `connectcoin.conf`        | User-defined [configuration settings](connectcoin-conf.md) for `connectcoind` or `connectcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `connectcoind.pid`        | Stores the process ID (PID) of `connectcoind` or `connectcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `connectcoind` or `connectcoin-qt`; can be specified by `-debuglogfile` option
...
`connectcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by ConnectCoin:
...
This table describes the files installed by ConnectCoin across different platforms.
...
| bin/connectcoin-qt                                             | ConnectCoin node and wallet GUI                                                 |
| bin/connectcoin-tx                                             | Tool for creating and modifying transactions                                |
...
There have been upstream reports of database corruption and data loss when using this filesystem with Bitcoin Core; the same inherited storage code is used here. See [upstream issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for details.
