...
Below are some notes on how to build ConnectCoin Core for Windows.

The options known to work for building ConnectCoin Core on Windows are:
...
example `/usr/src/connectcoin`, AND not under `/mnt/d/`. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\connectcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/connectcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/connectcoin --strip
```
