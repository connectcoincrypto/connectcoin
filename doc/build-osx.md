...
This guide describes how to build connectcoind, command-line utilities, and GUI on macOS.
...
in order to build ConnectCoin Core from source.
...
### 4. Clone the ConnectCoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the ConnectCoin Core repository to a directory.
...
You can [deploy](#3-deploy-optional) a `.zip` containing the ConnectCoin Core application.
...
Run the following in your terminal to compile ConnectCoin Core:
...
## Running ConnectCoin Core

ConnectCoin Core should now be available at `./build/bin/connectcoind`.
If you compiled support for the GUI, it should be available at `./build/bin/connectcoin-qt`.

There is also a multifunction command line interface at `./build/bin/connectcoin`
supporting subcommands like `connectcoin node`, `connectcoin gui`, `connectcoin rpc`, and
others that can be listed with `connectcoin help`.
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/ConnectCoin"

touch "/Users/${USER}/Library/Application Support/ConnectCoin/connectcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/ConnectCoin/connectcoin.conf"
```
...
```shell
./build/bin/connectcoind -daemon      # Starts the ConnectCoin daemon.
./build/bin/connectcoin-cli --help    # Outputs a list of command-line options.
./build/bin/connectcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./build/bin/connectcoin-qt -server # Starts the connectcoin-qt server mode, allows connectcoin-cli control
```
