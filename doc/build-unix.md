...
Some notes on how to build ConnectCoin Core in Unix.
...
memory available when compiling ConnectCoin Core. On systems with less, gcc can be
...
To build ConnectCoin Core without the wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
When the intention is to only run a P2P node, without a wallet, ConnectCoin Core can
