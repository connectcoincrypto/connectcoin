...
* [`BIP 324`](https://github.com/bitcoin/bips/blob/master/bip-0324.mediawiki): The v2 encrypted transport and associated `NODE_P2P_V2` service bit are supported. ConnectCoin extends the encrypted packet-length field from three to four bytes so the transport can carry its 50 MB protocol messages; it is therefore wire-compatible with ConnectCoin peers, not stock Bitcoin BIP324 peers.
