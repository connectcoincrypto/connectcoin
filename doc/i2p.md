# I2P support in ConnectCoin Core

It is possible to run ConnectCoin Core as an
...
## Run ConnectCoin Core with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://i2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with ConnectCoin Core:
...
ConnectCoin Core configuration options:
...
```
connectcoind -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `connectcoin-cli help
...
ConnectCoin inherits I2P support from upstream Bitcoin Core, and there may be fewer
I2P peers than Tor or IP ones. Therefore, using I2P alone without other networks may
make a node more susceptible to [Sybil
attacks](https://en.bitcoin.it/wiki/Weaknesses#Sybil_attack). You can use
`connectcoin-cli -addrinfo` to see the number of I2P addresses known to your node.
...
## Fetching I2P-related information from ConnectCoin Core
...
To see which I2P peers your node is connected to, use `connectcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `connectcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `connectcoin-cli help getnodeaddresses` for details.

## Compatibility

ConnectCoin Core uses the [SAM v3.1](https://i2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and ConnectCoin Core
...
To allow future upgrades to newer versions of SAM, ConnectCoin Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of ConnectCoin Core.
...
It's important that nodes of an application like ConnectCoin contribute
...
## Bundling I2P in a ConnectCoin application
