# bash programmable completion for connectcoin-cli(1)
# Copyright (c) 2012-present The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $connectcoin-cli for RPC
_connectcoin_rpc() {
    ...
    $connectcoin_cli "${rpcargs[@]}" "$@"
}

_connectcoin_cli() {
    local cur prev words=() cword
    local connectcoin_cli

    # save and use original argument to invoke connectcoin-cli for -help, help and RPC
    # as connectcoin-cli might not be in $PATH
    connectcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($connectcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_connectcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _connectcoin_cli connectcoin-cli
