...
be used in ConnectCoin Core with the `-asmap` option.
...
to binary before being used with ConnectCoin Core.
...
the `connectcoin-cli getnodeaddresses` command.
...
```
connectcoin-cli getnodeaddresses 0 > addrs.json
```
